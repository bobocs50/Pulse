// SOURCE: github.com/obro79/Rehabify/src/hooks/use-pose-detection.ts
// Main pose detection loop — reference for structure
//
// KEY DIFFERENCES FOR CPR:
//   1. They run detectForVideo() on main thread. We run it in a Web Worker
//      (otherwise audio scheduler jitters — our whole product is a metronome)
//   2. They use requestAnimationFrame. We use requestVideoFrameCallback
//      (fires once per decoded frame, not per display refresh — no duplicate GPU work)
//   3. Monotonically increasing timestamp hack (line ~68) — REQUIRED by MediaPipe, steal this
//   4. isActive flag on cleanup — steal this, prevents stale callbacks
  ...
  useEffect(() => {
    ...
    async function runVision(): Promise<void> {
      ...
      function processFrame(): void {
        ...
        // IMPORTANT: MediaPipe requires monotonically increasing timestamps
        ...
        if (!isPaused && exercise.form_detection_enabled && landmarks) {
          ...
          if (engine) {
            ...
            // Rep counting NOT throttled — must be immediate
